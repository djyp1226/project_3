$(document).ready(function () {
  ...
  $(window).on('resize', function () {
    ...
    if (winWidth > 767) { //pc일 경우
      ...
      _acco.find('.complex.on + .accopanel').show().css({
        width: panelWid
      });
    } else { //모바일일 경우
  ...
  $('#acco_box').find('.complex:first .accoheader').attr({
    'aria-expanded': true,
    'aria-disabled': true
  }).parent().siblings('.complex').children().attr('aria-expanded', false);

  //pc일 경우만 가로 크기를 0에서 지정한 사이즈로 넓혀 준다
  if ($('#acco_box').hasClass('pc')) {
    _acco.find('.accopanel:first').css({
      width: panelWid
    });
  }
  ...
  _acco.find('.complex .accoheader').on('click', function () {
    if (!$(this).parent().hasClass('on')) {
      ...
      $(this).attr({
        'aria-expanded': true,
        'aria-disabled': true
      }).parent().addClass('on').siblings('.complex.on').removeClass('on').children().attr('aria-expanded', false).removeAttr('aria-disabled');

      //아코디언 패널은 pc와 모바일 버전 각기 다르게 작성한다
      if ($('#acco_box').hasClass('pc')) {
        $(this).parent().next().css('display', 'block').stop().animate({
          width: panelWid
        }, 'slow').attr('tabIndex', 0).siblings('.accopanel').stop().animate({
          width: 0
        }, 'slow', function () {
  ...
  $(window).on('scroll', function () {
    ...
    timer = setTimeout(function () {
      ...
      //red_line
      if (scrollY > $(lineBox).offset().top - 100) $(lineBox).addClass('red_line');
